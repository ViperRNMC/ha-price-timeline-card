export default {
  no_data: "No data available for the day – are we still waiting for the prices?",
  no_data_attributes: "Your entity has no data attributes - see the readme",
  no_data_prices: "Your entity has no prices in the correct format - see the readme",
  missing_price: "Please specify 'price' sensor in the config",
  label_today_price: "Today's price",
  label_time_nextDay: "Price range",
  label_average_price: "Average Price",
  label_price: "Price",
  label_day: "Day",
  label_start: "Start",
  label_end: "End",
  label_duration: "Duration",
  label_name: "Name",
  unit_cent: "Cent",
  label_tomorrow_price: "Tomorrow's price",
  editor_theme_light: "Light",
  editor_theme_dark: "Dark",
  editor_theme_system: "System",
  editor_price_label: "Price sensor",
  editor_price_desc: "Sensor that contains the current price (in attribute data)",
  editor_average_label: "Optional: Average value",
  editor_average_desc: "Number (e.g. 0.225) indicating the average price (optional)",
  editor_theme_label: "Theme",
  editor_theme_desc: "Display mode of the card.",
  editor_start_today: "Today",
  editor_start_tomorrow: "Tomorrow",
  editor_start_yesterday: "Yesterday",
  editor_start_view_label: "Start View",
  editor_start_view_desc: "The view that should be displayed by default",
  editor_view_mode_label: "View Mode",
  editor_slider_label: "Slider",
  editor_slider_desc: "Show a slider to navigate through time slots",
  editor_graph_hours_label: "Graph Period (Hours)",
  editor_graph_hours_desc: "How many upcoming hours should be shown in graph view",
  editor_graph_start_from_label: "Graph from Now",
  editor_graph_start_from_desc: "When enabled the graph starts from now instead of local midnight",
  editor_graph_start_now: "Now",
  editor_currency_label: "Currency",
  editor_currency_desc: "Specify a custom currency",
  editor_color_scheme_label: "Color Scheme",
  editor_color_scheme_desc: "Choose how prices are colored",
  editor_gradient_fill_label: "Gradient Fill",
  editor_gradient_fill_desc: "Show a colored gradient fill under the graph line",
  editor_day_switch_label: "Day Switch",
  editor_day_switch_desc: "Shows or hides the day switch",
  editor_cheap_times_label: "Cheap Times",
  editor_cheap_times_desc: "Display of cheap time intervals",
  editor_cheapest_hour_color_label: "Cheapest Hour Color",
  editor_cheapest_hour_color_desc: "Optional CSS color for cheap-time highlighting in graph view (e.g. #00c896)",
  editor_display_settings: "Display Settings",
  editor_navigation_settings: "Period Settings",
  editor_price_optimization: "Price Optimization",
  editor_appearance_settings: "Appearance",
  editor_price_calculation: "Price Calculation",
  editor_enabled_label: "Enable Price Calculation",
  editor_enabled_desc: "Apply a formula on top of the raw market price sensor",
  editor_display_label: "Display Price As",
  editor_display_desc: "Show raw market price, all-in grid (consumption) or feed-in price",
  editor_price_calc_market: "Market price (raw)",
  editor_price_calc_grid: "Grid / Consumption (afname)",
  editor_price_calc_feed_in: "Feed-in / Teruglevering",
  editor_grid_addition_label: "Grid: Fixed addition (€/kWh)",
  editor_grid_addition_desc: "Fixed €/kWh added to market price for consumption (e.g. network + energy tax + supplier margin)",
  editor_grid_tax_label: "Grid: Tax / BTW (%)",
  editor_grid_tax_desc: "Percentage applied after addition, e.g. 21 for 21% BTW",
  editor_feed_in_addition_label: "Feed-in: Fixed addition (€/kWh)",
  editor_feed_in_addition_desc: "Fixed €/kWh adjustment for feed-in (negative = reduction, e.g. -0.02)",
  editor_feed_in_tax_label: "Feed-in: Tax / BTW (%)",
  editor_feed_in_tax_desc: "Percentage applied after addition for feed-in",
};